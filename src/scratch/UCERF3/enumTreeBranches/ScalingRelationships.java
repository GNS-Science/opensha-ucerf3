...
//			double w = xi*area/length; // unit of meters
	...
	/**
	 * This returns the slip (m) for the given rupture area (m-sq) or rupture length
	 * (m)
	 * 
	 * @param area      (m)
	 * @param length    (m)
	 * @param origWidth (m) - the original down-dip width (before reducing by
	 *                  aseismicity factor)
	 * @return
	 */
	...
	public static void makeSlipMagPlot(double downDipWidth, int maxLength, boolean saveFiles) {
		...
		for (int i = (int) downDipWidth; i <= maxLength; i++) {
			...
			double length = lengthKm * 1e3;
			double area = length * downDipWidth * 1e3;
			sh09_funcMod.set(sh09_Mod.getMag(area, downDipWidth * 1e3),
					sh09_Mod.getAveSlip(area, length, downDipWidth * 1e3));
			...
			sh12_sqrtL_func.set(ellB.getMag(area, downDipWidth * 1e3),
					sh12_sqrtL.getAveSlip(area, length, downDipWidth * 1e3));
			sh12_csd_func.set(sh09_Mod.getMag(area, downDipWidth * 1e3),
					sh12_csd.getAveSlip(area, length, downDipWidth * 1e3));
...
//    				"\t"+(float)hb.getMag(area, downDipWidth*1e3));
		...
		GraphWindow graph = new GraphWindow(funcs, "Implied Slip vs Mag Relationships; DDW=" + downDipWidth + " km",
				plotChars);
	...
	/**
	 * This tests the magnitudes and implied slip amounts for creeping-section
	 * faults assuming a length and DDW
	 */
